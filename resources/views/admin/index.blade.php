...
<div class="container mx-auto px-4">
    ...
    <a href="{{ route('product.create') }}" class="text-sm text-gray-600 hover:text-blue-600 mb-4 inline-block">Back to Home</a>
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        @foreach($products as $product)
            <div class="bg-white p-4 rounded-lg shadow hover:shadow-lg transition duration-300">
                <a href="{{ route('product.show', $product->id) }}">
                    <img src="{{ $product->images->first()->url ?? 'https://via.placeholder.com/150' }}" alt="{{ $product->name }}" class="w-full h-48 object-cover mb-4 rounded">
