...
<div class="min-h-screen bg-gray-50/50 dark:bg-gray-900 py-12 transition-colors duration-300">
    <div class="container mx-auto px-4 max-w-4xl">
        {{-- Back Button --}}
        <div class="mb-8">
            <a href="{{ route('chat.index') }}" class="inline-flex items-center gap-3 text-sm font-black uppercase tracking-widest text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all bg-white/80 dark:bg-gray-800/80 backdrop-blur-md px-6 py-3 rounded-2xl border border-white dark:border-gray-700 shadow-lg shadow-blue-900/5 dark:shadow-none group">
                <i class="fa-solid fa-arrow-left group-hover:-translate-x-1 transition-transform"></i>
                Kembali Ke Chat
            </a>
        </div>

        <div class="glass-panel rounded-[2.5rem] flex flex-col h-[75vh] bg-white dark:bg-gray-800 border border-white dark:border-gray-700 shadow-2xl shadow-blue-900/5 dark:shadow-none overflow-hidden">
            {{-- Chat Header --}}
            <div class="p-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between bg-white/50 dark:bg-gray-800/50 backdrop-blur-md relative z-10 shadow-sm">
                <div class="flex items-center gap-4">
                    <div class="w-14 h-14 rounded-2xl overflow-hidden border-2 border-white dark:border-gray-700 shadow-lg">
                        <img src="{{ $receiver->profile_photo_url }}" alt="{{ $receiver->name }}" class="w-full h-full object-cover">
                    </div>
                    <div>
                        <h2 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">{{ $receiver->name }}</h2>
                        <div class="flex items-center gap-1.5 mt-0.5">
                            <span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                            <span class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">Aktif Sekarang</span>
                        </div>
                    </div>
                </div>
                <div class="flex gap-2">
                    <form action="{{ route('chat.destroy', $receiver->id) }}" method="POST" onsubmit="return confirm('Hapus seluruh percakapan?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="w-10 h-10 rounded-xl flex items-center justify-center text-gray-400 dark:text-gray-500 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 transition-all" title="Hapus Chat">
            ...
            {{-- Messages Area --}}
            <div id="chat-messages" class="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-8 bg-gray-50/30 dark:bg-gray-900/30">
                @if($messages->isEmpty())
                    <div class="h-full flex flex-col items-center justify-center text-center py-20">
                        <div class="w-20 h-20 bg-blue-50 dark:bg-blue-900/30 rounded-full flex items-center justify-center mb-6">
                            <i class="fa-solid fa-handshake text-3xl text-blue-200 dark:text-blue-500"></i>
                        </div>
                        <h4 class="text-lg font-black text-gray-900 dark:text-white uppercase tracking-tight">Katakan Halo!</h4>
                        <p class="text-gray-400 dark:text-gray-500 font-medium text-sm mt-2 max-w-[240px]">Mulai percakapan dengan {{ $receiver->name }} untuk menanyakan produk ini.</p>
                    </div>
                @else
            ...
            {{-- Input Area --}}
            <div class="p-6 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 relative z-10 shadow-[0_-10px_30px_rgba(0,0,0,0.02)]">
                <form action="{{ route('chat.store') }}" method="POST" id="chat-form" class="flex gap-4">
                    ...
                    <div class="flex-1 relative">
                        <input type="text" name="message" id="chat-input" autocomplete="off" placeholder="Tulis pesan Anda di sini..." 
                               class="w-full bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-2xl px-6 py-4 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-600/20 focus:border-blue-600 transition-all placeholder-gray-400 dark:text-white" required>
                    ...
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white w-14 h-14 rounded-2xl flex items-center justify-center transition-all shadow-xl shadow-blue-100 dark:shadow-none active:scale-90 flex-shrink-0 group">
